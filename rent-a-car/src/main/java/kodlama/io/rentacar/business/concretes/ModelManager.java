...
public class ModelManager implements ModelService {
    ...
    //! Business rules
    private void checkIfModelExits(int id){
        if(!repository.existsById(id))
            throw new RuntimeException("Model mevcut değil");
    }
    private void checkIfGetAllSize(int size){
        if (size==0)throw new RuntimeException("Model bulunmamaktadır.");
    }
}
